...
def test_exact_ident_match_ranks_first(test_db, monkeypatch):
    monkeypatch.setenv("AIRFIELDOPS_DB_PATH", test_db)
    results = search_airports("KLAX")
    assert len(results) > 0
    assert results[0]["ident"] == "KLAX"
    assert results[0]["rank"] == 1

def test_iata_match_works(test_db, monkeypatch):
    monkeypatch.setenv("AIRFIELDOPS_DB_PATH", test_db)
...
def test_city_search_works(test_db, monkeypatch):
    monkeypatch.setenv("AIRFIELDOPS_DB_PATH", test_db)
    results = search_airports("Pittsburgh")
    # Both KAGC and KPIT should match via city
    idents = [r["ident"] for r in results]
    assert "KAGC" in idents
    assert "KPIT" in idents

def test_search_limit(test_db, monkeypatch):
    monkeypatch.setenv("AIRFIELDOPS_DB_PATH", test_db)
    results = search_airports("K", limit=2)
    assert len(results) == 2

def test_display_label_format(test_db, monkeypatch):
    monkeypatch.setenv("AIRFIELDOPS_DB_PATH", test_db)
    results = search_airports("KLAX")
