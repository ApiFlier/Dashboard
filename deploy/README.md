...
## Scripts
- **`setup.sh`**: Initializes the environment, creates `.env` from `.env.example`, discovers a free port, and starts the Docker container using a named volume. It optionally cleans up source files to leave a lightweight deployment footprint.
...
3. **Check Restart Policy:** Run `docker inspect airfieldops_prod | grep RestartPolicy -A 2`. It should show `unless-stopped`.
4. **Check Mounts:** Run `docker inspect airfieldops_prod | grep Mounts -A 10`. Ensure `Source` paths point to Docker volumes (`/var/lib/docker/volumes/...`) and not your local repository directory.
5. **Check Restart Resilience:** Stop the container (`docker stop airfieldops_prod`) and start it again (`docker start airfieldops_prod`). It should run perfectly without needing `docker-compose.yml`.
